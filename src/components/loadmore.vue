<template>
    <div class="try-loadmore">
        <div class="try-loadmore-wrapper" :class="{'is-dropped':topDropped || bottomDropped}" :style="{ 'transform': transform }">
            ...
            <slot></slot>
            <slot name="bottom">
                <div class="try-loadmore-bottom" v-if="bottomMethod">
                    <div v-if="bottomStatus === 'loading'" class="try-loadmore-spinner">加载更多...</div>
                    <span class="try-loadmore-text">{{ bottomText }}</span>
                </div>
            </slot>
        </div>
    </div>
    ...
    checkBottomReached() {
      if (this.scrollEventTarget === window) {
        ...
        return (
          ...
          parseInt(this.scrollEventTarget.getBoundingClientRect().bottom) + 1
        );
      }
    },
